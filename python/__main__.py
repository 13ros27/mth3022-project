...
def read_all_files(folder: str = '../modules', *, start: str = 'SYLLABUS PLAN - summary of the structure and academic content of the module', end: str = 'LEARNING AND TEACHING') -> dict[str, str]:
    return {f[f.find('MTH'):f.find('MTH')+7]: read_syllabus_plan(folder + '/' + f, start=start, end=end) for f in listdir(folder)}

if __name__ == '__main__':
    words_map = {k: words.split(v) for (k, v) in read_all_files().items()}
    depths = {}

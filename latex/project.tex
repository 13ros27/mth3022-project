...
There are multiple different types of both OWL and OWL2, designed for different use cases. OWL2 has a general specification `OWL2 DL'\cite{w3c_owl2} and then there exist profiles which remove some of the complexity to make them easier to use for particular domains, while still remaining fully compatible with `DL'. These profiles are `OWL2 EL' which is designed for relatively simple reasoning over very large datasets, `OWL2 QL' which allows queries about relationships between items to be answered very efficiently, for example in databases, and `OWL2 RL' which allows the user to reason directly on RDF triples.
...
Each vertex has a unique id. (a unique name). Two vertecies of different graphs {\it match} when they have the same id.
...
$$\bold A_{i,j} = \begin{cases}1&\text{if}\; i\sim j,\\0&\text{otherwise.}\end{cases}$$
...
The {\it degree matrix} $\bold D$ is an eye matrix whose diaonal is composed by the degree of each vertex.

$$\bold D_{i,j} =  \begin{cases}d_i&\text{if}\; i = j,\\0&\text{otherwise.}\end{cases}$$

\paragraph{Spectrum of a matrix}

The {\it spectrum of a matrix} is a sorted sequence of eigenvalues.\\
Let us denote the $i-$th eigenvalue of the adjacency matrix by $\lambda_i^{\bold A}$, and the $i-$th eigenvalue of the laplacien matrix by $\lambda_i^{\bold L}$

\paragraph{Adjacency and Laplacian spectral distance}
Let $\lambda^{\bold A_A}$ and  $\lambda^{\bold A_B}$ be the adjacency spectra of the graphs $A$ and $B$ and let $\lambda^{\bold L_A}$ and  $\lambda^{\bold L_B}$ be their laplacien spectra.

Then, the {\bf Adjacency spectral distance} is defined as :

$$d_{\bold A}(A,B) = \Bigg( \sum_{i=1}^n (\lambda_i^{\bold A_A} - \lambda_i^{\bold A_B})^2\Bigg)^{1/2}$$
